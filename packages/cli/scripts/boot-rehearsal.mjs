...
async function run() {
    ...
    // Run simulation
    const worker = new Worker(path.join(__dirname, 'rehearsal-worker-boot.mjs'), {
        workerData: {
            contestStage,
            numRuns: runsArg
        }
    ...
    worker.on('message', (msg) => {
        if (msg.type === 'progress') {
            ...
            const totalResult = results.find(r => r.id === 'TOTAL_SCORE');
            const individualResults = results.filter(r => r.id !== 'TOTAL_SCORE');

            // Assign idol name back to result based on loadout.id
            const finalIdols = individualResults.map((r, index) => {
            ...
            const finalData = {
                ...
                runs: runsArg,
                idols: finalIdols,
                totalScore: Math.floor(totalResult.score),
                totalMin: totalResult.min,
                totalQ1: totalResult.q1,
                totalMedian: totalResult.median,
                totalQ3: totalResult.q3,
                totalMax: totalResult.max
            };
