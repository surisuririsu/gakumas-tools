...
- Multiple choice, toggles and filters: separate `chip`s with a gap; selected ones use `chip-selected`. The current pick in a grid of icons gets `selected-ring`, with no tilt. Anything that clips or outlines a game icon uses `$radius-icon`, which matches the frame drawn into the art. Quiet text links use `link-pill`.
